...
class Folder extends Component {
    ...
    return (
      <>
        ...
        <Row className="my-4 px-md-4">
          <Col md={8}>
            {this.props.screen == "recentActivities" ? null : (
              <Divider orientation="left" className="count-divider" plain>
                <span className="element-count">
                  Tổng có:{" "}
                  <span className="font-number">
                    {this.state[this.props.screen]?.paginate?.total_items || 0}
                  </span>
                </span>
              </Divider>
            )}
        ...
        {this.state.filter != "recentActivities" ? (
          <Row className="mt-5">
            <Col md={8} className="d-flex justify-content-center">
              <Pagination
                count={Math.ceil(
                  data?.paginate?.total_items / data?.paginate?.items_per_page
                )}
                color="primary"
                size="large"
  ...
  renderData(data) {
    if (this.props.screen == "made" || this.props.screen == "learned") {
      return <>{data?.sets?.map((ele) => this.renderContent(ele))}</>;
    } else if (this.state.filter == "folder") {
      return <>{data?.folders?.map((ele) => this.renderContent(ele))}</>;
    }
  }
