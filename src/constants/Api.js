...
function createAxios() {
  ...
  axiosInstant.defaults.baseURL =
    "https://flashcard-web-project.herokuapp.com/api/";
...
export const requestFolders = (payload) => {
  return handleResult(
    getAxios.get(`listFolders?current_page=${payload.page}`, {
...
export const requestFolderDetail = (payload) => {
  return handleResult(
    getAxios.get(
      `folderDetail?id=${payload.id}&current_page=${payload.current_page}`
    )
  );
};
...
export const requestRecentSets = (payload) => {
  return handleResult(
    getAxios.get(`recentSets?current_page=${payload.page}`, { ...payload })
  );
};

export const requestLearnedSet = (payload) => {
  return handleResult(
    getAxios.get(`set/completed?current_page=${payload.page}`)
  );
};
...
export const requestGame = (payload) => {
  return handleResult(
    getAxios.get(`multipleChoiceGame?id=${payload.id}`, payload)
  );
};
...
export const requestSetNoFolder = (payload) => {
  return handleResult(
    getAxios.get(
      `set/no-folder?current_page=${payload.page}&folder_id=${payload.folder_id}`
    )
  );
};
